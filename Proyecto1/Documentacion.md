...
2. El uso del caché puede llegar a ser bastante provechoso, ya que estos pueden ayudar a disminuir el consumo de ancho de banda.
3. El equilibrio de carga puede ser útil si se desea evitar sobrecargar una página y/o servidor. No obstante, Round Robin no es el mejor método para esto. 
4. Tener un proxy reverso puede aligerar la carga de nuestros servidores de sobremanera, además de ser fáciles de implementar.
5. Docker es una herramienta muy útil para virtualizar redes locales.
6. Un web server daemon puede ser muy útil para una implementación sencilla y rápida.
7. traceroute nos permite ver los hops que se realizan entre routers, cuando se realiza una petición a un endpoint, esto es muy útil, pues podemos ver si existe algún hop innecesario que afecte el rendimiento.
